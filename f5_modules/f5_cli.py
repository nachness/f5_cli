from f5_modules.f5_update_controller import F5Manager, NodeController, \
...
def take_node_action(args):
    ...
    for node in args.nodes_list:
        node_object = NodeController(node=node,arg_parser=args)
        ...
        get_node_status(node_object)

def get_node_status(node_object):
...
def take_rules_action(args):
    ...
    for vs_name in args.vip_list:
        vs_object = VirtualServerController(vip_name=vs_name,
                                            arg_parser=args)
 
        if args.remove:
            vs_object.remove(args.rule_name)
        elif args.add:
            vs_object.add(args.rule_name)
        else:
            print("No add or remove specified, what are you trying to do?")
        
        print vs_object
        rules_list = vs_object.get_vip_attr("rules")
